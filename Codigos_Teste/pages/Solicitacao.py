...
def alterar_status_biblioteca(json_path, scope, LOGIN, SITUACAO, ID_LIVRO, data):
    if SITUACAO == 'Emprestado':
        ...
        sheet = spreadsheet.worksheet("Teste")  # Nome da aba
...
#=== 5º Passo - Sistema de Recomendação ===#
def sistema_recomendacao(json_path, scope, ID_LIVRO, RECOMENDACAO, data):
    if RECOMENDACAO == '👍':
        linha = data.loc[data['ID_LIVRO'].astype(int) == int(ID_LIVRO)]
        if linha['RECOMENDACAO'].values[0] == '':
            data.loc[data['ID_LIVRO'].astype(int)==int(ID_LIVRO), 'RECOMENDACAO'] = '1 👍'
        else:
            valor_recomendacao = linha['RECOMENDACAO'].values[0].split(" ")[0]
            print(f'\nvalor recomendação: {valor_recomendacao}')
            valor_recomendacao = int(valor_recomendacao) + 1
            data.loc[data['ID_LIVRO'].astype(int)==int(ID_LIVRO), 'RECOMENDACAO'] = f'{valor_recomendacao} 👍'
        ...
        # Atualizar a planilha com os novos dados
        sheet.update(data_atualizada)
    print("\n✅ Recomendação salva com sucesso!")
    return
    ...
    if st.session_state.SITUACAO_USUARIO == 'Empréstimo' and st.session_state.dados.loc[int(st.session_state.ID_LIVRO)-1,'SITUACAO'] == 'Disponível':
        ...
        if st.session_state.dados.loc[int(st.session_state.ID_LIVRO)-1,'SITUACAO'] == 'Emprestado':
            if st.session_state.dados.loc[int(st.session_state.ID_LIVRO)-1,'FUNCIONARIO'].strip() == st.session_state.LOGIN.strip():
                ...
                st.write("")
                #=== Criar formulário ===#
                with st.form(key='recomendacao'):
                    RECOMENDACAO = st.selectbox('Você recomenda a leitura deste livro?', options=['👍', '👎'])
                    input_buttom_submit_avaliacao = st.form_submit_button("Avaliar")

                    if input_buttom_submit_avaliacao:
                        st.session_state.RECOMENDACAO = RECOMENDACAO
                        sistema_recomendacao(json_path=json_path, scope=scope, 
                                        ID_LIVRO=st.session_state.ID_LIVRO, 
                                        RECOMENDACAO=st.session_state.RECOMENDACAO, 
                                        data=st.session_state.dados)
                        st.write("")
                        st.success("Obrigado por avaliar! ☺️")
            else:
